...
Перед установкой должен существовать маршрут к обоим DNS IP, а сетевая политика должна разрешать TCP и UDP 53.
...
проверьте lookup внутри приложения; централизованной альтернативой служит split-DNS для нужной зоны.
...
Без маршрута к указанным DNS-серверам имена MDB будут завершаться ошибкой или тайм-аутом. Остальные DNS-зоны эта
